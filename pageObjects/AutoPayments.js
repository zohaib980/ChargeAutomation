...
export class AutoPayments {
    ...
    gotoAutopayments() {
        ...
        cy.get('[href*="/payment-rules"][class="dropdown-item"]').should('contain.text', 'Auto Payments').click() //Auto Payments
        cy.get('.page-title').should('contain.text', 'Account Setup').and('be.visible')
    ...
    validateAddBookingSource() {
        ...
        cy.get('[class="toast toast-error"]').should('contain.text', 'Booking Source name is required') //error as fields are empty
        cy.get('.loading-label').should('not.exist') //loader should be disappear
    ...
    validateSelectedSettingsOnAllBS() {
        cy.get('[class*="card sourceBooking-card"]').each((ele, index) => {
            cy.get('[class*="card sourceBooking-card"] [id*="headingOne"]').eq(index).find('span').invoke('text').then(text => {
                cy.get('.loading-label').should('not.exist') //loader should be disappear
                cy.get('[id*="editAutoPayments"]').eq(index).should('be.visible').click() //Open modal : edit icon
                cy.get('.show [class="modal-content autopayment-modal"]').should('be.visible') //Modal window
                if (text.includes('Credit Card')) {
                    cy.get('[id*="cardValidation"] [id*="OnOffForSource"]').should('be.checked')
                    cy.log('Credit Card Validation is enabled for this BS')
                } else {
                ...
                cy.wait(1000)
                cy.get('[id*="closeAPDetailsmodal"]').should('be.visible').should('contain.text', 'Cancel').click().wait(500) //Close modal
                cy.get('.show [class="modal-content autopayment-modal"]').should('not.exist').wait(500) //Modal window should be closed
            })
        })
                ...
                .find('[id*="OnOffForSource"]').then(toggle => {
                    if (toggle.prop('checked')) {
                        ...
                        cy.get('[class*="card sourceBooking-card"] .logo-title').contains(BSource).then(ele => {
                            ...
                            cy.verifyToast('Booking Source Settings Updated')
    ...
    enableReservationPayment(bSource, amountTypePayment, amountTypePaymentSelect, reservation_authTime, reservation_chargeOption) {
        ...
        cy.get('.show [id*="paymentSchedule"] [id*="OnOffForSource"]').then(toggle => { //Toggle status
            ...
            if (toggle.prop('checked')) {
                ...
                cy.verifyToast('Booking Source Settings Updated') //Success toast
            } else {
        ...
        cy.get('[class*="card sourceBooking-card"] .logo-title').contains(bSource1).then(ele => {
            cy.wrap(ele).parents('[class*="card sourceBooking-card"]')
                .find('[id*="headingOne"]').invoke('text').then(text => {
                    ...
                    cy.get('[id*="closeAPDetailsmodal"]').should('contain.text', 'Cancel').and('be.visible').wait(500).click() //Close modal
                })
        })
    }
    copyAPSettings(OldBS, newBS) {
        ...
        cy.get('.toast-text').should('contain.text', 'AP settings were successfully copied')
    }
